...
class Member : public Customer {
    ...
    public:
         ...
         void setYear(){
            valid_year = timePtr->tm_year + 1900 + 1; 
         }
...
class Vendor {
	private:
		...
		float rating;
	public:
		Vendor(string name, float rating, int numMovies){
		...
		void calculateBill(){
			float pay = 100000; // 100000 price per screening
			float discount;
			int screens;
			if(allowVendor()){
				cout << "How many screenings do you want \n\n";
				cin >> screens;
				if(rating == 10 && numMovies > 5)
					discount = 0.5;
				
				else if(rating > 7.5 && numMovies > 5)
					discount = 0.25;
				
				else discount = 1;
				
				pay *= discount * screens;
				cout << "Your Total Bill for your Screenings is " << pay << endl;
			}
			else cout << "Sorry! You cannot screen at out Cinema\n\n";
		}
};
...
int main()
{
    ...
    do{
        ...
        if (selection_role < 'a' || selection_role > 'g')
            cout << "                                         You have entered a wrong selection. Try again.\n\n";
    }while(selection_role < 'a' || selection_role > 'g');
    ...
    if (selection_role == 'a'){
       movie_booking();
        ...
        management();
    }else if(selection_role == 'f'){
    	Vendor V1("Christopher Nolan",10.0,11);
    	V1.calculateBill();
	}else{
        exit(1);
    }
...
void title()
{
    const int WIDTH = 88;
    const int WIDTH1 =  18;
    const string heading =  "SEAT SAVVY - ULTIMATE MOVIE COMPANION" ;
    cout << "\n\n ";
...
void payment(float pay)
{
    ...
    if (resp == 'A' || resp == 'a'){
        ...
        cout << " The amount you need to pay is : RS " << pay << endl << endl;
    }
...
seats:
	...
	cout << "\nThere are two types of seats available:  (A) DELUXE (RS 1500) \n"
		<< "\t\t\t\t\t (B) NORMAL (RS 900)\n\n\n"
	...
	if(response == 'a'){
		cout << "Congrats! You will receive Member Discount\n\n";
		if (seat_type == 'a')
			seat_price = 1200 * seat_no;
	
		else if (seat_type == 'b')
			seat_price = 700 * seat_no;	

	}
	
	else if(response == 'b'){
		if (seat_type == 'a')
			seat_price = 1500 * seat_no;
		
		else if (seat_type == 'b')
			seat_price = 900 * seat_no;	
...
receipt:
	...
	cout << "\nPrice of seat(s) : " << "RS " << seat_price << endl << endl;
	cout << "---------------------------------------------------------" << endl
	...
	booking << movie_name[select_movie] << '|' << movie_day[select_movie] << '|' << showtime << '|' << seat_type << '|' << seat_price << '|';
...
void management()
{
    ...
    while (loginAttempt < 5)
    {
        ...
        if (userName == "Hassan" && userPassword == pass){
            cout << "Welcome Hassan!" << endl;
            break;
        }
        else if (userName == "Mishaal" && userPassword == pass){
            cout << "Welcome Mishaal!" << endl;
            break;
        }
        else if (userName == "Shaheer" && userPassword == pass){
            cout << "Welcome Shaheer!" << endl;
            break;
        }
        else{
    ...
    while (flag){
        ...
        cout << "                                          (4) Check booking history " << endl << endl << endl;
...
void ticket_cancel(){
    ...
    char ticket_num[25], name[25], phone[25], email[40], seat_type[25], movie[25], day[25], showtime[25], seat_booked[60], seat_price[10];
